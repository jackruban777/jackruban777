<!--
  JACK RUBAN P — Premium GitHub Profile README
  Replace only:
  YOUR_USERNAME
  YOUR_LINKEDIN_URL
  YOUR_EMAIL
  YOUR_REPO_URL_1
  YOUR_REPO_URL_2
  YOUR_REPO_URL_3
-->

<!-- ═══════════════════════════════════════════════════════════════ -->
<!-- HERO                                                            -->
<!-- ═══════════════════════════════════════════════════════════════ -->

<p align="left">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=220&color=0:6C3BFF,50:00D9FF,100:FF3CAC&text=JACK%20RUBAN%20P&fontSize=42&fontColor=ffffff&fontAlignY=38&desc=SOFTWARE%20DEVELOPER&descSize=18&descAlignY=58&animation=fadeIn" width="100%" />
</p>

<h1 align="left">JACK RUBAN P</h1>

<h3 align="left">
  SOFTWARE DEVELOPER
</h3>

<p align="left">
  <b>Building intelligent applications • Exploring modern technologies • Creating immersive web experiences</b>
</p>

<br>

<p align="left">

<a href="https://github.com/YOUR_USERNAME">
<img src="https://img.shields.io/badge/GitHub-111111?style=for-the-badge&logo=github&logoColor=ffffff" />
</a>

&nbsp;

<a href="YOUR_LINKEDIN_URL">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=ffffff" />
</a>

&nbsp;

<a href="mailto:YOUR_EMAIL">
<img src="https://img.shields.io/badge/Email-FF3CAC?style=for-the-badge&logo=gmail&logoColor=ffffff" />
</a>

&nbsp;

<img src="https://komarev.com/ghpvc/?username=YOUR_USERNAME&style=for-the-badge&color=6C3BFF&label=PROFILE+VIEWS" />

</p>

<br>

<p align="left">
  <img src="https://skillicons.dev/icons?i=java,python,js,html,css,react,nodejs,threejs&perline=8" />
</p>

<br>

<p align="left">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=15&duration=2800&pause=900&color=00D9FF&left=true&vCenter=true&width=500&lines=Writing+Clean+Code;Building+Real+Applications;Exploring+Modern+Technology;Learning+Every+Day" />
</p>

---

# 👋 ABOUT ME

### Hello, I'm Jack

I'm a **Software Developer** passionate about transforming ideas into useful, modern and engaging digital experiences.

I enjoy working across different areas of software development, from designing interfaces and building applications to exploring artificial intelligence, cloud technologies and interactive web experiences.

### What I Build

- 💻 Software applications
- 🌐 Full-stack applications
- 🤖 AI-powered applications
- 🎨 Modern UI/UX experiences
- 🧊 Interactive 3D websites
- ☁️ Cloud applications
- 🛠️ Developer tools

<br>

> **My approach:** Learn continuously, build consistently and turn ideas into working software.


# ⚙️ TECH STACK

### 💻 Languages

<p align="cener">
  <img src="https://skillicons.dev/icons?i=java,python,js,html,css" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/SQL-111111?style=for-the-badge&logo=postgresql&logoColor=00D9FF" />
</p>

<br>

### 🌐 Frontend Development

<p align="center">
  <img src="https://skillicons.dev/icons?i=react,threejs,tailwind" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Three.js-111111?style=for-the-badge&logo=threedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/React%20Three%20Fiber-111111?style=for-the-badge&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/GSAP-111111?style=for-the-badge&logo=greensock&logoColor=88CE02" />
</p>

<br>

### ⚡ Backend Development

<p align="left">
  <img src="https://skillicons.dev/icons?i=nodejs,express" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/REST%20APIs-111111?style=for-the-badge&logo=fastapi&logoColor=00D9FF" />
  <img src="https://img.shields.io/badge/Backend%20Development-111111?style=for-the-badge&logo=nodedotjs&logoColor=68A063" />
</p>

<br>

### 🗄️ Database

<p align="center">
  <img src="https://skillicons.dev/icons?i=supabase,postgresql" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Database%20Design-111111?style=for-the-badge&logo=postgresql&logoColor=00D9FF" />
  <img src="https://img.shields.io/badge/API%20Integration-111111?style=for-the-badge&logo=supabase&logoColor=3ECF8E" />
</p>

<br>

### 🤖 AI & Machine Learning

<p align="center">
  <img src="https://skillicons.dev/icons?i=python" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Generative%20AI-6C3BFF?style=for-the-badge&logo=google-gemini&logoColor=white" />
  <img src="https://img.shields.io/badge/Computer%20Vision-00D9FF?style=for-the-badge&logo=opencv&logoColor=white" />
  <img src="https://img.shields.io/badge/AI%20Applications-FF3CAC?style=for-the-badge&logo=google&logoColor=white" />
</p>

<br>

### ☁️ Cloud & DevOps

<p align="center">
  <img src="https://skillicons.dev/icons?i=docker,git,github" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/APIs-111111?style=for-the-badge&logo=fastapi&logoColor=00D9FF" />
  <img src="https://img.shields.io/badge/Deployment-111111?style=for-the-badge&logo=vercel&logoColor=white" />
  <img src="https://img.shields.io/badge/Cloud-111111?style=for-the-badge&logo=icloud&logoColor=00D9FF" />
</p>

<br>

### 🧰 Development Tools

<p align="center">
  <img src="https://skillicons.dev/icons?i=vscode,git,github,docker,postman" />
</p>

<br>

### 🎮 Creative Web Technologies

<p align="center">
  <img src="https://skillicons.dev/icons?i=threejs,react,tailwind" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/GSAP-88CE02?style=for-the-badge&logo=greensock&logoColor=111111" />
  <img src="https://img.shields.io/badge/3D%20Web-6C3BFF?style=for-the-badge&logo=threedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Interactive%20UI-FF3CAC?style=for-the-badge&logo=framer&logoColor=white" />
</p>

---

# 🤖 AI & DEVELOPMENT TOOLS

### 🚀 Google Antigravity

**AI-assisted development environment**

A modern environment I use for exploring faster development workflows.

**Used for**

`Architecture` · `Coding` · `Debugging` · `Rapid Prototyping`

<br>

### 🧠 Google AI Studio

**Generative AI development platform**

Used for experimenting with Gemini-powered applications and AI workflows.

**Exploring**

`Gemini API` · `AI Applications` · `Prompt Engineering`

<br>

### ✨ Gemini

**Generative AI technology**

Exploring how modern AI can be integrated into practical software applications.

**Areas**

`Coding` · `Learning` · `Generative AI` · `AI Applications`

<br>

### 🎨 Stitch

**AI-powered UI/UX exploration**

Used for exploring interface ideas, layouts and design systems.

**Areas**

`UI Design` · `UX` · `Prototyping` · `Design Systems`

---

# 🚀 FEATURED PROJECTS

## 📚 LearnFlow

### AI-Powered Adaptive Learning Platform

An AI-powered platform that generates personalized learning roadmaps based on the learner's selected technical skill.

### Features

- AI-generated roadmaps
- Micro-concept learning
- Adaptive quizzes
- Progress tracking
- Authentication
- Supabase database
- Modern UI

### Built With

<p align="left">
  <img src="https://skillicons.dev/icons?i=react,nodejs,express,supabase,tailwind" />
</p>

`React` · `Node.js` · `Express` · `Supabase` · `Gemini` · `Tailwind CSS`

<br>

<a href="https://github.com/jackruban777/LearnFlow">
<img src="https://img.shields.io/badge/View%20Project%20→-6C3BFF?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br>

<a href="YOUR_REPO_URL_2">
<img src="https://img.shields.io/badge/View%20Project%20→-00A8CC?style=for-the-badge&logo=github&logoColor=white" />
</a>

---

## 🌙 Night Vision AI

### Computer Vision Project

A computer vision project focused on detecting objects in low-light environments.

### Features

- Object detection
- Low-light vision
- Fast inference
- Camera processing
- AI detection

### Built With

<p align="left">
  <img src="https://skillicons.dev/icons?i=python,opencv" />
</p>

`Python` · `YOLO` · `OpenCV` · `AI / ML`

<br>

<a href="https://github.com/jackruban777/NIGHT-VISION-AI">
<img src="https://img.shields.io/badge/View%20Project%20→-FF3CAC?style=for-the-badge&logo=github&logoColor=white" />
</a>

---

# 🧠 CURRENTLY LEARNING

### ☕ Java

`DSA` · `OOP` · `Algorithms` · `Problem Solving` · `Advanced Java`

<br>

### 🤖 AI

`Generative AI` · `Gemini` · `AI Applications` · `Prompt Engineering` · `Computer Vision`

<br>

### 🌐 Web

`React` · `Three.js` · `React Three Fiber` · `GSAP` · `Modern UI/UX`

<br>

### ☁️ Cloud

`APIs` · `Deployment` · `Databases` · `Cloud Architecture` · `DevOps`



# 💡 MY DEVELOPMENT MINDSET

### 💡 IDEATE

Understand the problem, explore possibilities and define the direction.

↓

### 🎨 DESIGN

Plan the architecture, interface and overall user experience.

↓

### 💻 BUILD

Write clean, maintainable and scalable code.

↓

### 🧪 TEST

Validate the application, find issues and improve reliability.

↓

### 🚀 DEPLOY

Turn the project into a usable and accessible product.

↓

### 🔁 IMPROVE

Keep learning, experimenting and making the product better.

---

# ❤️ WHAT I LOVE BUILDING

### 💻 SOFTWARE APPLICATIONS

Practical software designed to solve real problems and create useful experiences.

<br>

### 🌐 FULL-STACK APPLICATIONS

Complete applications combining modern frontend interfaces with reliable backend systems.

<br>

### 🎨 INTERACTIVE UI

Modern interfaces with motion, depth, animation and thoughtful interactions.

<br>

### ☁️ CLOUD SYSTEMS

Connected applications, APIs, databases and deployment workflows.

---

# 🌐 LET'S CONNECT

### 💼 LinkedIn

Connect with me and follow my development journey.

<a href="www.linkedin.com/in/jackruban777">
<img src="https://img.shields.io/badge/Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<br><br>

### 💻 GitHub

Explore my projects, experiments and code.

<a href="[https://github.com/YOUR_USERNAME](https://github.com/jackruban777)">
<img src="https://img.shields.io/badge/Explore-111111?style=for-the-badge&logo=github&logoColor=white" />
</a>

<br><br>

### 📧 Email

Let's build something together.

<a href="jackruban777@gmail.com">
<img src="https://img.shields.io/badge/Contact-FF3CAC?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

---

<p align="center">

## `BUILD • LEARN • CREATE • REPEAT`

**Thanks for visiting my profile!**

<br>

⭐ **If you like my work, consider starring one of my repositories.**

</p>

<br>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:6C3BFF,50:00D9FF,100:FF3CAC" width="100%" />
</p>
